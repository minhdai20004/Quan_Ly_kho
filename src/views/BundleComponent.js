...
const StockBar = ({qty, min, max}) => {
  ...
  const color   = qty===0 ? C.red : qty<(min||0) ? C.amber : C.green;
  return (
...
const StockBadge = ({qty, min}) => {
  ...
  if(qty<(min||0))   return <span style={{display:'inline-flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:700,background:C.amberL,color:C.amber,border:'1px solid rgba(217,119,6,0.25)'}}><Warning size={11} weight="fill"/>Sắp hết</span>;
...
function AdjustModal({open, stock, warehouses, onClose, onSaved}) {
  ...
  if(!open) return null;
  const matName = stock?.material_id?.material_name||stock?.material_id?.product_name||'—';
  const matCode = stock?.material_id?.material_code||stock?.material_id?.product_code||'';
    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()}
      ...
      <div style={{background:C.surface,borderRadius:24,padding:'0 0 24px',width:460,boxShadow:'0 24px 64px rgba(0,0,0,0.16)',border:`1px solid ${C.border}`,animation:'inv-scalein 0.3s cubic-bezier(0.16,1,0.3,1)',overflow:'hidden'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 22px',background:'linear-gradient(135deg,#fdf2f8,#fce7f3)',borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:'flex',alignItems:'center',gap:11}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:C.pink,marginTop:1}}>Cập nhật số lượng thực tế</div>
            </div>
          </div>
        ...
        <div style={{padding:'20px 22px',display:'flex',flexDirection:'column',gap:16}}>
          ...
          {/* Qty */}
          <div style={{display:'flex',flexDirection:'column',gap:5}}>
            <label style={{fontSize:11.5,fontWeight:700,color:C.text2,textTransform:'uppercase',letterSpacing:'0.5px'}}>Số lượng tồn mới <span style={{color:C.pink}}>*</span></label>
            <input type="number" min={0} value={qty} onChange={e=>setQty(e.target.value)}
              style={{...inSx(),fontSize:22,fontWeight:800,textAlign:'center',letterSpacing:'-0.5px',fontFamily:'JetBrains Mono,monospace'}}
              onFocus={focusPink} onBlur={blurReset}/>
            {diff!==0&&(
...
export default function Inventory() {
  ...
  const load = useCallback(async()=>{
    ...
    try {
      ...
      setStocks(Array.isArray(res?.data)?res.data:Array.isArray(res)?res:[]);
    } catch { setStocks([]); }
  ...
  const filtered = stocks.filter(s=>{
    ...
    const name=(s.material_id?.material_name||s.material_id?.product_name||'').toLowerCase();
    const code=(s.material_id?.material_code||s.material_id?.product_code||'').toLowerCase();
    return name.includes(q)||code.includes(q);
  });

  const outOfStock = stocks.filter(s=>(s.quantity_on_hand||0)===0).length;
  const lowStock   = stocks.filter(s=>s.low_stock_alert&&s.quantity_on_hand>0).length;
  const totalCost  = stocks.reduce((sum,s)=>sum+(s.total_cost||0),0);
  ...
  const STATS = [
    ...
    {label:'Giá trị tồn kho',value:fmtCur(totalCost),  unit:'',         color:C.sky,   bg:C.skyL,   Icon:CurrencyDollar, accentTop:`linear-gradient(90deg,${C.sky},#38bdf8)`, big:true},
  ];
      ...
      <div style={{background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:'0 4px 20px rgba(0,0,0,0.05)',overflow:'hidden',animation:'inv-fadeup 0.4s cubic-bezier(0.16,1,0.3,1)'}}>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:13.5}}>
          ...
          <tbody>
            {loading ? (
              ...
              const name  = s.material_id?.material_name||s.material_id?.product_name||'—';
              const code  = s.material_id?.material_code||s.material_id?.product_code||'';
              const wName = s.warehouse_id?.warehouse_name||s.warehouse_id?.name||'—';
              const qty   = s.quantity_on_hand||0;
              const avail = s.quantity_available??qty;
              const minSt = s.material_id?.min_stock||s.min_stock||0;
              ...
              return (
                <tr key={s._id}
                  ...
                  <td style={{padding:'13px 14px',textAlign:'right',fontSize:13,color:C.text3,fontFamily:'JetBrains Mono,monospace'}}>
                    {minSt>0?fmt(minSt):'—'}
                  </td>
                  <td style={{padding:'13px 14px',textAlign:'right'}}>
                    <span style={{fontFamily:'JetBrains Mono,monospace',fontWeight:700,fontSize:13,color:cost>0?C.sky:C.text3}}>{cost>0?fmtCur(cost):'—'}</span>
                  </td>
        ...
        {!loading&&filtered.length>0&&(
          <div style={{padding:'13px 18px',borderTop:`1px solid ${C.border2}`,fontSize:13,color:C.text3,background:'#fafbfc',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            ...
            <span>Tổng giá trị: <strong style={{color:C.sky,fontFamily:'JetBrains Mono,monospace'}}>{fmtCur(filtered.reduce((s,x)=>s+(x.total_cost||0),0))}</strong></span>
          </div>
        )}
      </div>
